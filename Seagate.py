...
def connect_client(kv_records):
    ...
    client = session.resource('s3', region_name='US', endpoint_url='http://s3.seagate.com')
...
def count_words(filename, num_print):
    ...
    with open(filename) as file:
        ...
        for lines in content:
            line = lines.split('\n')[0]
            # Provides a list of words in each line.
            ...
            for word in words_in_line:
                for stop_word in stop_words:
                    ...
                    # Check if word is a word to be eliminated.
                    if (word_to_check in file_words.keys()):
                        file_words[word_to_check] += 1
                    else:
                        file_words[word_to_check] = 1
                    # If so, omit the character
                    if (word_to_check in word_counts.keys()):
                        word_counts[word_to_check] += 1
                    else:
                        word_counts[word_to_check] = 1
            
    file_words_map[filename] = file_words
    ...
    # printFileWord(file_words, num_print)
    printFileWord(word_counts, num_print)
    # print(word_counts)
    return word_counts

# sorting most popular words of a dict and print
def printFileWord(word_counts, num_print):
    sort_orders = sorted(word_counts.items(), key=lambda x: x[1], reverse=True)
    n = min(num_print, len(word_counts.keys()))
...
def process_fdmi_record(kv_record, num_print):
    ...
    try:
        ...
        kv = {'fid': kvs["fid"], 'cr_key': str_decode(kvs["cr_key"]), 'cr_val': str_decode(kvs["cr_val"])}
...
def poll_directory(path, words, sleep_seconds):
    ...
    try:
        while True:
            ...
            sleep(sleep_seconds)  # TODO this should be a parameter with a default value of 1
...
def main(args):
    ...
    if args.local_directory:
        print('Listening for emulated FDMI events on:\n  {}'.format(args.local_directory), file=sys.stderr)
        poll_directory(args.local_directory, args.num_print, args.sleep_seconds)
    else:
